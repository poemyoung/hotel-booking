...
public class SignService {
    ...
    public boolean userSignUp(String phone,String pwd){
        if(usersMapper.selectByPhone(phone) > 0){   //用户已存在
            return false;
        }
        //随机生成用户名
    ...
    public boolean adminSignUp(String hotel,String pwd){
        if(adminsMapper.selectByHotelName(hotel) > 0){  //admin已存在
            return false;
        }
        //随机生成hotelID
        int id;
        ...
        while (true) {
            ...
            if(hotelsMapper.selectByPrimaryKey(id)== null)break;    //当生成的id不与hotels中已有任何id重合时完成
        }
